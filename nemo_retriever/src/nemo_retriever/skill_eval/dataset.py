...
def load_eval_manifest(path: Path) -> list[DatasetEntry]:
    ...
    for idx, item in enumerate(data, start=1):
        ...
        candidates = (
            (item.get("sdg_prompt_candidates") or item.get("scenario_prompt_candidates") or {}).get("candidates") or []
        )
        selected_variant = (
            item.get("sdg_prompt_validation") or item.get("scenario_prompt_validation") or {}
