...
from nv_ingest.schemas.chart_extractor_schema import ChartExtractorConfigSchema, ChartExtractorSchema  # Adjust the import as per your file structure
...
def test_invalid_config_with_empty_services():
    with pytest.raises(ValidationError) as excinfo:
        ChartExtractorConfigSchema(
            cached_endpoints=(None, None),
            deplot_endpoints=(None, None),
...
def test_invalid_endpoint_format():
    with pytest.raises(ValidationError):
        ChartExtractorConfigSchema(
            cached_endpoints=("invalid_endpoint", None),
