...
def run(
    ...
    query_file: Path = typer.Option(
        ..., "--query-file", exists=True, file_okay=True, dir_okay=False, help="File with one query per line."
    ),
    ...
    console.print(
        f"[bold cyan]Stage8[/bold cyan] images={df_query.shape[0]} input_dir={query_file} device={device} hf_cache_dir={hf_cache_dir}"
    )
    ...
    for query_batch in tqdm(query_texts, desc="Stage8 query embeddings", unit="queries"):
        tokenized_inputs = tokenizer(["query: " + query_batch], return_tensors="pt", padding=True, truncation=True).to(
            device
        )
        embed_model_results = embed_model(**tokenized_inputs)
        emb = (
            average_pool(embed_model_results.last_hidden_state, tokenized_inputs["attention_mask"])
            .detach()
            .cpu()
            .numpy()
        )
    ...
    results_df, hit_summary = analyze_query_hits(golden_answers, retrieved_pdf_pages, query_texts, ks=[1, 3, 5, 10])
