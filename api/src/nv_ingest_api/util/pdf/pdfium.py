...
def convert_bitmap_to_corrected_numpy(bitmap: pdfium.PdfBitmap) -> np.ndarray:
    ...
    Uses fancy indexing on the zero-copy view to fuse the channel swap and copy into
    a single allocation.
    ...
    """
    mode = bitmap.mode
    view = bitmap.to_numpy()  # zero-copy view of C buffer

    if mode in {"BGRA", "BGRX"}:
        return view[..., [2, 1, 0, 3]]  # 1 copy: swap + materialize
    elif mode == "BGR":
        return view[..., [2, 1, 0]]  # 1 copy: swap + materialize
    else:
        return view.copy()  # already RGB/L; just copy
