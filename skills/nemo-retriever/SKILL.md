...
This skill targets engine **contract_version 1.6.0** (`contract/cli-contract.json`). The engine can also be driven over MCP (`retriever mcp`, stdio) for harnesses that prefer MCP tools (`query`, `verify`) over CLI calls. On the setup turn, after the index is built, verify the installed engine matches:
